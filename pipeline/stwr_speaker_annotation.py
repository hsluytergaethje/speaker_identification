...
from preprocess import preprocess_text, get_sentences, initialize_flair
from Text import create_text_class
from evaluate import get_eval_per_class, evaluate_stwr_gold_annotations, join_evaluations
...
def get_text_objects(text, stwr_recog_dir, parzu_path, model, postprocess, verb_path, mensch_path, vocative_path):
	from STWR_recognition import STWRecognizer, STWRFeatureExtractor
	os.chdir(stwr_recog_dir)
	stwr_clf = STWRecognizer(mode='test', model=model, feature_extractor=STWRFeatureExtractor(sequence_features=True))

	stw_text = "\n".join(text)
	parzu_text = " ".join(text)
	with tempfile.NamedTemporaryFile() as tmpf:
		stw_name = tmpf.name
		stwr_clf.annotate(stw_text, filename_output=stw_name, postprocess=postprocess, html=False)
		stw_name_out = stw_name + ".tsv"
		all_text_objects = preprocess_text(parzu_text, stw_name_out, parzu_path, verb_path, mensch_path, vocative_path)
	os.chdir(CWD)
	return all_text_objects

# preprocess text and annotations, create text objects 
def get_text_eval_object_from_annotated_text(filepath, stwr_recog_dir, parzu_path, model, postprocess, 
	...
	text_objects_stwr_predicted = get_text_objects(text, stwr_recog_dir, parzu_path, model, postprocess, \
...
def process_text(path_to_input, outputfile, mode, parsed_sieves, parsed_classifier_paths, stwr_path, parzu_path, model, postprocess,
	...
	if mode == "annotate":
		...
		stw_type_to_object = get_text_objects(text, stwr_path, parzu_path, model, postprocess, verb_path, mensch_path, vocative_path)
		...
		stw_type_to_object, true_objects = get_text_eval_object_from_annotated_text(path_to_input, stwr_path, parzu_path, model, \
			postprocess, verb_path, mensch_path, vocative_path)
		if stw_type_to_object == 0:
			return 0
		print("Speaker Annotation")
...
def main():
	...
	# import stwr annotation module 
	stwr_path = config["STW_Recognition"]["stw_tool_path"]
	sys.path.insert(1, stwr_path)
	

	model = config["STW_Recognition"]["model"]
	postprocess = config["STW_Recognition"]["postprocess"]
	...
	for filename in args.input:
		...
		try:
			evaluation = process_text(filename, outputfilename, args.mode, parsed_sieves, parsed_classifier_paths, stwr_path, parzu_path, \
				model, postprocess, verb_path, mensch_path, vocative_path, speech_noun_path, functional_verb_path, print_eval)
			if evaluation == 0:
				continue
	...
	if len(collected_evaluations) >= 1:
		print("="*70)
		join_evaluations(collected_evaluations)
