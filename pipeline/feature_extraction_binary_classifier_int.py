...
Feature values are restricted to booleans and numbers. 
"""
...
#ACCUSATIVE_DEPENDENCY_LABEL = "OA"
#DATIV_DEPENDENCY_LABEL = "DA"
...
def get_distance_features(text, annotation, candidates, context):
	...
	for i in range(len(candidates)):
		...
		features["punc_inbetween_context_to_quote"] = int(punctuation_inbetween[i])
		"""
...
def get_candidate_features(text, annotation, candidates, context):
	...
	candidate_pos = [POS_MAPPING[candidate.pos] for candidate in candidates]
	...
	pos_of_previous = [POS_MAPPING[token.pos] if token != "-" else -1 for token in previous_tokens]
	pos_of_next = [POS_MAPPING[token.pos] if token != "-" else -1 for token in next_tokens]

	previous_is_punc = [token.is_punctuation() if token != "-" else -1 for token in previous_tokens]
	next_is_punc = [token.is_punctuation() if token != "-" else -1 for token in next_tokens]
	...
	# instead check if speech verb 
	parent_verb_is_speech_verb = [recursive_verb_head_search(candidate).is_speech_verb() if recursive_verb_head_search(candidate) \
	 is not None else False for candidate in candidates]
	...
	for i in range(len(candidates)):
		...
		features["in_conversation"] = int(candidate_in_conversation)

		features["candidate_order"] = candidate_orders[i]
		features["candidate_in_quote"] = int(candidate_in_quote[i])
		features["candidate_in_quote_sentence"] = int(candidate_in_quote_sentence[i])
		
		# mention features
		features["pos"] = candidate_pos[i]
		features["is_pronoun"] = int(candidate_is_pronoun[i])
		features["is_subject"] = int(candidate_subjects[i])
		features["is_acc"] = int(candidate_accs[i])
		features["is_dativ"] = int(candidate_dativs[i])
		#features["lemma_of_verb_parent"] = candidate_parent_verb_lemma[i]
		features["parent_verb_is_speech_verb"] = int(parent_verb_is_speech_verb[i])
		
		# features of surrounnding token
		features["pos_previous"] = pos_of_previous[i]
		features["pos_next"] = pos_of_next[i]
		features["previous_is_punc"] = int(previous_is_punc[i])
		features["next_is_punc"] = int(next_is_punc[i])
		features["previous_in_quote"] = int(previous_in_quote[i])
		features["next_in_quote"] = int(next_in_quote[i]) 
...
def get_quote_features(text, annotation, candidates, context):
	...
	for i in range(len(candidates)):
		...
		features["candidate_name_in_quote"] = int(candidate_name_in_quote[i])
		features["annotation_is_split"] = int(annotation.is_split())
		features["first_word_in_quote_is_lower"] = int(first_word_is_lower)
...
def get_all_int_features(text, annotation, candidates, context):
